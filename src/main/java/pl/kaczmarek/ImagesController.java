...
public class ImagesController {
    ...
    @Value("${imageFilesPathJs}")
    private String imageFilesPathJs;

    @Value("${imageFilesPathCss}")
    private String imageFilesPathCss;
    ...
    @ResponseBody
    @RequestMapping(value = "/jsX/{name}",
        method = RequestMethod.GET, produces = MediaType.ALL_VALUE)
    public ResponseEntity<Object> getJs(@PathVariable("name") String name) throws IOException {
        InputStream in = new FileInputStream(imageFilesPathJs + name);
        HttpHeaders headers = new HttpHeaders();
        return new ResponseEntity<Object>(IOUtils.toByteArray(in), headers, HttpStatus.OK);
    }

    @ResponseBody
    @RequestMapping(value = "/cssX/{name}",
        method = RequestMethod.GET, produces = MediaType.ALL_VALUE)
    public ResponseEntity<Object> getCss(@PathVariable("name") String name) throws IOException {
        InputStream in = new FileInputStream(imageFilesPathCss + name);
        HttpHeaders headers = new HttpHeaders();
        return new ResponseEntity<Object>(IOUtils.toByteArray(in), headers, HttpStatus.OK);
    }
